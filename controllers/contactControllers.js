...
const getContact= async(req,res)=>{
    try{const contact=await Contact.find()
...
const updateContact= async(req,res)=>{
    try{const contact=await Contact.findByIdAndUpdate({_id:req.params.id},{...req.body})
...
const deleteContact= async(req,res)=>{
    try{const contact=await Contact.findByIdAndDelete(req.params.id)
        if(!contact)return res.status(404).json({msg:"contact not found"})
